...
@Component
@Slf4j
public class ActivityLogSessionManager {
    ...
    public void register(Integer studentId, WebSocketSession session) {
        ...
        log.debug("Registered WebSocket session {} for student {} (activity logs)", session.getId(), studentId);
    }
    ...
    public void remove(WebSocketSession session) {
        ...
        log.debug("Removed WebSocket session {} from student {} (activity logs)", session.getId(), studentId);
    }

    public void broadcast(Integer studentId, String payload) {
        Set<WebSocketSession> sessions = studentSessions.get(studentId);
        if (sessions == null || sessions.isEmpty()) {
            log.debug("No active activity log sessions for student {}", studentId);
            return;
        }
        ...
        for (WebSocketSession session : sessions) {
            try {
                if (session.isOpen()) {
                    ...
                    log.debug("Sent activity log to student {} session {}", studentId, session.getId());
                }
            } catch (IOException e) {
                log.warn("Failed to send activity log message to session {}", session.getId(), e);
            }
        }
    }
}
